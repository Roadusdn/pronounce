...
def get_all_attempts(user_id: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for attempt_dir in ATTEMPTS_DIR.iterdir():
        if not attempt_dir.is_dir():
            continue
        summary_file = attempt_dir / "attempt.json"
        if summary_file.exists():
            try:
                attempt = _read_json(summary_file)
                if user_id is None or attempt.get("user_id") == user_id:
...
def save_analysis_result(
    ...
    return update_attempt(
        attempt_id,
        {
            ...
            "consonant_score": comparison.get("consonant_score"),
            "vowel_score": comparison.get("vowel_score"),
...
def clear_attempts(user_id: Optional[str] = None) -> int:
    if not ATTEMPTS_DIR.exists():
        return 0
